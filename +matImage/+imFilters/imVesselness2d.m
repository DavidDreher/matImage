...
if length(sigma) > 1
    ...
    for i = 1:nSigmas
        [V(:,:,i), B(:,:,i), S(:,:,i)] = matImage.imFilters.imVesselness2d(img, sigma(i), beta, c);
    end
...
[gxx, gxy, gyy] = matImage.imFilters.imHessian(double(img), sigma);

% compute eigen values of hessian matrix. 
% They are sorted such that |lambda1| < |lambda2|
s2 = sigma ^ 2;
[lambda1, lambda2] = matImage.imFilters.imEigenValues(gxx*s2, gxy*s2, gyy*s2);
