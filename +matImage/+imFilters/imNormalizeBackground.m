...
if ndims(img) > 2 && size(img, 3) == 3 %#ok<ISMAT>
    [r, g, b] = matImage.imFilters.imSplitBands(img);
    [r, bgR, rmseR] = matImage.imFilters.imNormalizeBackground(r, varargin{:});
    [g, bgG, rmseG] = matImage.imFilters.imNormalizeBackground(g, varargin{:});
    [b, bgB, rmseB] = matImage.imFilters.imNormalizeBackground(b, varargin{:});
    res = matImage.imFilters.imMergeBands(r, g, b);
    bg  = matImage.imFilters.imMergeBands(bgR, bgG, bgB);
...
if nargin < 2
    bin = img > matImage.imFilters.imOtsuThreshold(img);
else
    bin = varargin{1};
end
