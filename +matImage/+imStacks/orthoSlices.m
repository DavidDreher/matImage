...
siz = matImage.imStacks.stackSize(img);
...
% extract each slice
sliceXY = squeeze(matImage.imStacks.stackSlice(img, 3, pos(3)));
sliceZY = squeeze(permute(matImage.imStacks.stackSlice(img, 1, pos(1)), [2 1 3]));
sliceXZ = squeeze(permute(matImage.imStacks.stackSlice(img, 2, pos(2)), [2 1 3]));
...
if display3d
    ...
    [hSlice3dXY hSlice3dYZ hSlice3dXZ] = matImage.imStacks.orthoSlices3d(img, pos);
...
% extract each slice
sliceXY = squeeze(matImage.imStacks.stackSlice(img, 3, pos(3)));
sliceZY = squeeze(matImage.imStacks.stackSlice(img, 1, pos(1)));
sliceXZ = squeeze(matImage.imStacks.stackSlice(img, 2, pos(2)));
