...
methods
    function this = OrthoSlicer3d(img, varargin)
        ...
        box = matImage.imStacks.stackExtent(this.imageData, spacing, origin);
...
methods
    ...
    function hs = createSlice3d(this, dim, index, varargin)
        ...
        % extract the slice
        slice = matImage.imStacks.stackSlice(this.imageData, dim, this.position(dim));
    ...
    function dragSlice(this, src, event) %#ok<INUSD>
        ...
        index = min(max(1, index), matImage.imStacks.stackSize(this.imageData, data.dim));
        ...
        slice = matImage.imStacks.stackSlice(this.imageData, data.dim, this.sliceIndex);
