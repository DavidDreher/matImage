...
handles = resetImageData(handles);
img = matImage.imStacks.readstack(filename);
set(handles.imageDisplay, 'userdata', img);
...
handles = resetImageData(handles);
set(handles.imageDisplay, 'userdata', matImage.imStacks.metaImageRead(info));
...
img = matImage.imStacks.readstack(fullfile(pathname, filename), dataType, dims([2 1 3]));
...
[img inds] = matImage.imStacks.rotateStack90(img, axis, n);
...
figure();
matImage.imStacks.orthoSlices(get(handles.imageDisplay, 'userdata'), pos, spacing, ...
...
% create figure with 3 orthogonal slices
figure();
matImage.imStacks.orthoSlices3d(get(handles.imageDisplay, 'userdata'), pos, spacing, ...
    'DisplayRange', handles.grayscaleExtent, 'lut', handles.lut);
