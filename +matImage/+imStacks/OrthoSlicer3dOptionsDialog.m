...
methods
    function onApplyButtonClicked(this, hObject, eventdata) %#ok<INUSD>
        ...
        if rotateXAxis
            imgData = matImage.imStacks.flipStack(matImage.imStacks.stackRotate90(imgData, 'x', 1), 2);
        ...
        cmap = [];
        if ~matImage.imStacks.isColorStack(imgData) && ~isempty(this.parent.colorMap)
            cmap = this.parent.colorMap;
        end
        
        % create figure with 3 orthogonal slices in 3D
        figure();
        matImage.imStacks.OrthoSlicer3d(imgData, 'Position', pos, ...
            'Origin', origin, 'Spacing', spacing, ...
            'DisplayRange', this.parent.displayRange, 'ColorMap', cmap);
        
        % compute display extent (add a 0.5 limit around each voxel)
        extent = matImage.imStacks.stackExtent(imgSize, spacing, origin);
