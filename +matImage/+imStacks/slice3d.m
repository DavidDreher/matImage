...
% extract the slice
slice = matImage.imStacks.stackSlice(img, dim, index);
...
index = min(max(1, index), matImage.imStacks.stackSize(data.img, data.dim));
data.index = index;
...
slice = matImage.imStacks.stackSlice(data.img, data.dim, data.index);
