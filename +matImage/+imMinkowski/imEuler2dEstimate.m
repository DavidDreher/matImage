...
if ~islogical(img)
    ...
    for i = 1:length(labels)
        chi(i) = matImage.imMinkowski.imEuler2dEstimate(img == labels(i), varargin{:});
    end
    return;
end
...
chi     = matImage.imMinkowski.imEuler2d(img, conn);

% compute EPC on each border of image, and keep the average
chix    = mean([ matImage.imMinkowski.imEuler1d(img(1,:)) matImage.imMinkowski.imEuler1d(img(end,:)) ]);
chiy    = mean([ matImage.imMinkowski.imEuler1d(img(:,1)) matImage.imMinkowski.imEuler1d(img(:,end)) ]);
