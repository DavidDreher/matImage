...
if ~islogical(img)
    ...
    for i = 1:length(labels)
        chi(i) = matImage.imMinkowski.imEuler3dEstimate(img==labels(i), varargin{:});
    end
    return;
end
...
chi     = matImage.imMinkowski.imEuler3d(img, varargin{:});

% compute EPC on each of the 12 border edge of image, and keep the average
chix    = mean([ ...
    matImage.imMinkowski.imEuler1d(img(:,   1,   1)) ...
    matImage.imMinkowski.imEuler1d(img(:, end,   1)) ...
    matImage.imMinkowski.imEuler1d(img(:,   1, end)) ...
    matImage.imMinkowski.imEuler1d(img(:, end, end)) ...
    ]);
chiy    = mean([ ...
    matImage.imMinkowski.imEuler1d(img(  1, :,   1)) ...
    matImage.imMinkowski.imEuler1d(img(end, :,   1)) ...
...
chixy    = mean([ ...
    matImage.imMinkowski.imEuler2d(squeeze(img(:, :,   1)), conn2d) ...
    matImage.imMinkowski.imEuler2d(squeeze(img(:, :, end)), conn2d) ...
    ]);
chixz    = mean([ ...
    matImage.imMinkowski.imEuler2d(squeeze(img(:,  1,  :)), conn2d) ...
    matImage.imMinkowski.imEuler2d(squeeze(img(:, end, :)), conn2d) ...
